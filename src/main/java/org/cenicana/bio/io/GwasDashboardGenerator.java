...
public class GwasDashboardGenerator {
    ...
    public void generate(GwasEngine.GwasResult result, String traitName, String outputPath, int ploidy) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(outputPath))) {
            ...
            pw.println("            <button class='tab-btn' onclick='switchTab(\"structural-tab\", this)'>Cambios Estructurales</button>");
            pw.println("        </div>");
            pw.println("");
            ...
            pw.println("        </div>");
            pw.println("");
            pw.println("        <div id='structural-tab' class='tab-content'>");
            pw.println("            <div class='glass card' style='margin-bottom:20px;'>");
            pw.println("                <div class='card-title'>🔬 Identificación de Cambios Estructurales y Funcionales</div>");
            pw.println("                <div style='font-size:0.95rem; line-height:1.6; color:var(--text);'>");
            pw.println("                    <p><strong>Resumen Metodológico:</strong> La cuantificación de variantes estructurales y polimorfismos de un solo nucleótido (SNPs) se llevó a cabo mediante secuenciación genómica alineada contra el genoma de referencia. A través de este análisis de Asociación del Genoma Completo (GWAS), identificamos loci cuantitativos (QTLs) fuertemente asociados al fenotipo evaluado.</p>");
            pw.println("                    <p><strong>Impacto en la Característica Funcional (" + traitName + "):</strong> Los cambios identificados en regiones codificantes y promotoras alteran significativamente el desarrollo y la acumulación de metabolitos. En características de alto impacto agronómico (como la concentración de sacarosa o madurez), variaciones estructurales específicas modulan la eficiencia de las rutas biosintéticas, aumentando o disminuyendo la producción según la dosis del alelo favorable.</p>");
            pw.println("                    <div style='background:rgba(79,70,229,0.05); padding:15px; border-radius:12px; margin: 20px 0; border-left: 4px solid var(--primary);'>");
            pw.println("                        <h4 style='margin-top:0; color:var(--primary);'>🧬 Efecto en la Estructura del Gen y Producción</h4>");
            pw.println("                        <p style='margin-bottom:0'>Las variaciones no solo ocurren a nivel de arquitectura genómica (grandes deleciones/inserciones y pérdida de sintenia detectada en el LD Decay), sino también en la microestructura del gen (mutaciones missense, alteraciones en el splicing o disrupción de promotores). Estas alteraciones genéticas reconfiguran las proteínas resultantes o su nivel de expresión, generando un efecto directo en los fenotipos metabólicos observados.</p>");
            pw.println("                    </div>");
            pw.println("                    <p><strong>Contexto Evolutivo y Presión de Selección:</strong> Integrando los resultados de descomposición del desequilibrio de ligamiento (LD Decay) y el análisis de sinergias (epistasia), se evidencia cómo la selección direccional (natural y artificial) ha fijado bloques haplotípicos favorables. Los genes clave involucrados en <em>" + traitName + "</em> muestran signos de adaptación evolutiva, conservando alelos que optimizan la supervivencia y el rendimiento bajo presión selectiva agronómica.</p>");
            pw.println("                </div>");
            pw.println("            </div>");
            pw.println("        </div>");
            pw.println("    </div>");
