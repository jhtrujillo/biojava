...
Finalmente, el sistema inyecta estas coordenadas en tres algoritmos complementarios de agrupamiento no supervisado: *K-Means* (centroides geométricos), *DBSCAN* (densidad espacial y remoción de ruido/híbridos anómalos) y Modelos de Mezcla Gaussiana (GMM, para estimar probabilities de hibridación gradual), visualizándolos en paneles interactivos 3D.
...
Este módulo convierte archivos de variantes complejas a matrices cuantitativas de dosis alélicas adaptadas a herencia polisómica. Para ello, el software lee secuencialmente el archivo VCF depurado y utiliza el **vector de estados de dosis** discretas $\mathbf{d} = [0, 1, 2, \dots, \text{ploidía}]$ para aproximar y mapear la dosis matemática esperada de cada muestra hacia el espacio real de ploidía de la especie bajo estudio. El comando integra parámetros clave para modular esta exportación: el archivo VCF de entrada (`-v` o `--vcf`), la ploidía genómica (`--ploidy`), los hilos asignados (`-t` o `--threads`) para un procesamiento de flujo paralelo ultrarrápido, y la bandera para exportar la dosis esperada continua calculada (`--raw`); permitiendo asimismo aproximar las dosis a valores discretos enteros correspondientes al vector de dosis según los requerimientos analíticos del investigador y exportando los resultados en formato tabular (`.tsv`) de alto rendimiento.
```bash
